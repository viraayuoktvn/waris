...
def decision_tree_without_pruning(X_train, y_train, X_test, y_test, criterion='gini'):
    dt_classifiers = []
    accuracies = []
    precisions = []
    recalls = []
    ...
    for i in range(y_train.shape[1]):
        dt_classifier = DecisionTreeClassifier(criterion=criterion, splitter="best", random_state=42)
...
def decision_tree_with_prepruning(X_train, y_train, X_test, y_test, criterion='gini', max_depth=None, min_samples_split=2):
    dt_classifiers = []
    accuracies = []
    precisions = []
    recalls = []
    ...
    for i in range(y_train.shape[1]):
        dt_classifier = DecisionTreeClassifier(criterion=criterion, splitter="best", max_depth=max_depth,
...
def decision_tree_with_pruning(X_train, y_train, X_test, y_test, criterion='gini', ccp_alpha=0.0):
    dt_classifiers = []
    accuracies = []
    precisions = []
    recalls = []
    ...
    for i in range(y_train.shape[1]):
        dt_classifier = DecisionTreeClassifier(criterion=criterion, splitter="best", ccp_alpha=ccp_alpha, random_state=42)
        dt_classifier.fit(X_train, y_train.iloc[:, i])
        dt_classifiers.append(dt_classifier)
        y_pred = dt_classifier.predict(X_test)
        accuracy = accuracy_score(y_test.iloc[:, i], y_pred)
...
def decision_tree_with_prepruning_pruning(X_train, y_train, X_test, y_test, criterion='gini', max_depth=None, min_samples_split=2,
                                          ccp_alpha=0.0):
    dt_classifiers = []
    accuracies = []
    precisions = []
...
dt_classifiers, accuracies, precisions, recalls = decision_tree_without_pruning(X_train, y_train, X_test, y_test, criterion='gini')
...
dt_classifiers, accuracies, precisions, recalls = decision_tree_with_prepruning(X_train, y_train, X_test, y_test, criterion='gini', max_depth=10, min_samples_split=10)
for i, dt_classifier in enumerate(dt_classifiers):
    tree_rules = export_text(dt_classifier, feature_names=list(X.columns))
...
dt_classifiers, accuracies, precisions, recalls = decision_tree_with_pruning(X_train, y_train, X_test, y_test, criterion='gini', ccp_alpha=0.01)
for i, dt_classifier in enumerate(dt_classifiers):
    tree_rules = export_text(dt_classifier, feature_names=list(X.columns))
...
dt_classifiers, accuracies, precisions, recalls = decision_tree_with_prepruning_pruning(X_train, y_train, X_test, y_test, criterion='gini', max_depth=10, min_samples_split=10, ccp_alpha=0.01)
for i, dt_classifier in enumerate(dt_classifiers):
    tree_rules = export_text(dt_classifier, feature_names=list(X.columns))
