...
def dt_c45(Xdata, ydata, pre_pruning, post_pruning, chi_lim=0.1, min_lim=10):
            ...
            if is_number(dataset.iloc[:, element]) == True:
                ...
                for bin_split in range(start, finish):
                    ...
                    if i > 0 and pre_pruning == "chi_2" and chi_squared_test(branch[i].iloc[:, 0], bin_sample[0]) > chi_lim:
                        if ";" not in rule[i]:
                            rule[i] = rule[i] + " THEN " + name + " = " + branch[i].agg(
                                lambda x: x.value_counts().index[0])[0] + ";"
                            rule[i] = rule[i].replace(" AND  THEN ", " THEN ")
                        skip_update = True
                        continue
    ...
    if post_pruning:
        ...
        for r in rule[:-2]:  # Exclude the "Total Number of Rules" and "Default Value" rules
            ...
            print(r)  # Print all rules, including pruned ones
...
y = df[['hw_cl']]  # Target, remain as string

start_time = time.time()
dt_model = dt_c45(Xdata=X, ydata=y, pre_pruning="none", post_pruning=True)
end_time = time.time()
